...
df1 = pd.read_csv('/Users/saave/Desktop/FONDEF_DATA/Entrenamiento_LOCAL/LSTM_LOCAL2.CSV')
df1['date'] = pd.to_datetime(df1['date'])

#####################################################################################################
############################ PARA ENTRENAR POR CLUSTERS #############################################
#####################################################################################################
...
ids_para_eliminar = df1[df1['caudal_mask2_p0d'] == 3]['gauge_id'].unique()
# Filtrar el DataFrame para excluir esos gauge_id
df1 = df1[~df1['gauge_id'].isin(ids_para_eliminar)]
print(df1)
...
###################################################
potencia = 4 
df1['caudal_mean_p0d'] = ((86.4*df1['caudal_mean_p0d'])/df1['top_s_cam_area_tot_b_none_c_c'])**(1/potencia)
...
def create_and_save_dataset(df_original, df_filtrado, time_steps, prediction_days, dataset_name, dir_path):
    ...
    with h5py.File(f'{dir_path}/Secuencias_{dataset_name}/{dataset_name}.h5', 'w') as hf:
        ...
        print('=' * 35)
        print(' '*10 + f'{dataset_name}-{gauge_id}')
        print('=' * 35)
        ...
        if conteo_creado == 0:
         error_message = f"Se han creado cero secuencias para el gauge_id {dataset_name.split('_')[-1]}"
         print(error_message)
         raise ValueError(error_message)  # Levanta una excepción si no se crearon secuencias
...
# -------------------------------------------------------- #
# Función para procesar cada cuenca individualmente
# -------------------------------------------------------- #

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# Diccionario para almacenar el número de secuencias
num_sequences_dict = {}

# Función para procesar cada cuenca individualmente
def process_gauge_data(gauge_id, train, val, test, dir_path, time_steps, prediction_days):
    logging.info(f'Procesando cuenca {gauge_id}')
    # Crear las rutas de directorios para secuencias y escaladores
    sequences_path = os.path.join(dir_path, 'Secuencias', f'{gauge_id}')
    scalers_path = os.path.join(dir_path, 'Scaler', f'{gauge_id}')

    # Asegurarse de que los directorios existen
    os.makedirs(sequences_path, exist_ok=True)
    os.makedirs(scalers_path, exist_ok=True)

    # Filtrar datos por gauge_id
    train_gauge = train[train.index.get_level_values('gauge_id') == gauge_id]
    val_gauge = val[val.index.get_level_values('gauge_id') == gauge_id]
    test_gauge = test[test.index.get_level_values('gauge_id') == gauge_id]

    # Filtrar el DataFrame de características globales SIN ESCALAR para obtener solo las de la cuenca actual !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    df2_current = df2[df2.index.get_level_values('gauge_id') == gauge_id]
    
    # Escalado de datos
    train_scaled, val_scaled, test_scaled, df2_current, X_scaler, y_scaler = scale_data(train_gauge, val_gauge, test_gauge, target='caudal_mean_p0d', df_original=df2_current)

    # Guardar los escaladores
    with open(os.path.join(scalers_path, f'X_scaler_{gauge_id}.pkl'), 'wb') as file:
        pickle.dump(X_scaler, file)
    with open(os.path.join(scalers_path, f'y_scaler_{gauge_id}.pkl'), 'wb') as file:
        pickle.dump(y_scaler, file)

    # Creación de secuencias
    num_sequences_train = create_and_save_dataset(df_original = df2_current, df_filtrado=train_scaled, time_steps=time_steps, prediction_days=prediction_days, dataset_name=f'train', dir_path=sequences_path)
    num_sequences_val = create_and_save_dataset(df_original = df2_current, df_filtrado=val_scaled, time_steps=time_steps, prediction_days=prediction_days, dataset_name=f'val', dir_path=sequences_path)
    num_sequences_test = create_and_save_dataset(df_original = df2_current, df_filtrado=test_scaled, time_steps=time_steps, prediction_days=prediction_days, dataset_name=f'test', dir_path=sequences_path)

    # Guardar el número de secuencias en el diccionario
    num_sequences_dict[gauge_id] = {
        'train': num_sequences_train,
        'val': num_sequences_val,
        'test': num_sequences_test
    }

    logging.info(f'Secuencias guardadas para {gauge_id}: Entrenamiento={num_sequences_train}, Validación={num_sequences_val}, Prueba={num_sequences_test}')
    return num_sequences_train, num_sequences_val, num_sequences_test
...
time_steps = 250 
prediction_days = 5
for gauge_id in df1.index.get_level_values('gauge_id').unique():
    process_gauge_data(gauge_id, train, val, test, dir_path, time_steps = time_steps, prediction_days = prediction_days)

# Guardar el diccionario de secuencias en un archivo
with open(f'{dir_path}/num_sequences.pkl', 'wb') as file:
    pickle.dump(num_sequences_dict, file)
